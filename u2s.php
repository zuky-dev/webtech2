  ...
  $timId=$_GET['idT'];
    ...
    if ($resultT->num_rows > 0) {
      ...
      if ($conn->query($sql11) === TRUE) {
        header( "refresh:1;url=u2s.php?id=" . $_POST['idStudenta'] );
      } else {
    ...
    if( $prideleneBody>0){
      header( "refresh:2;url=u2s.php?id=" . $_POST['idStudenta'] );
...
if(isset($_POST['yes'])){
    ...
    $sql11 = "UPDATE studentTeam SET confirmStudent=TRUE WHERE  student_id='". $_POST['idStudenta']."' 
    AND team_id='".$timId."'";
    if ($conn->query($sql11) === TRUE) {
      header( "refresh:0;url=u2s.php?id=". $_POST['idStudenta'] );
    } else {
	...
	if(isset($_POST['no'])){
    $timId=$_POST['idTeam'];
    $sql11 = "UPDATE studentTeam SET confirmStudent=FALSE WHERE  student_id='". $_POST['idStudenta']."'
    AND team_id='".$timId."'";
    if ($conn->query($sql11) === TRUE) {
      header( "refresh:0;url=u2s.php". $_POST['idStudenta'] );
    } else {
      echo "Počas aktualizovania údajov sa vyskytla chyba.";
    }
...
<head>
  ...
  <script src="//ajax.googleapis.com/ajax/libs/jquery/1.9.1/jquery.min.js"></script>
   ...
   <link rel="stylesheet" href="assets/css/styleUploadPoints.css">
  <script>
      function suhlas() {
...
if ($result1->num_rows > 0) {
  ...
  while($row1 = $result1->fetch_assoc()) {
    ...
    echo'<div class="container">
<form class="form-horizontal" method="post" action="u2s.php?idSt='.$idstud.'&idT='.$idtim.'">
  <input type="hidden" name="idStudenta" value="'.$id.'">
...
echo'
<form class="form-horizontal" method="post" action="u2s.php?">
  <div class="form-group"> 
    <div class="col-sm-offset-2 col-sm-4 ">
      <input type="hidden" name="idStudenta" value="'.$id.'">
      <input type="hidden" name="idTeam" value="'.$idtim.'">
